...
def index_document(path: str, chunk_size: int = 400, overlap: int = 80) -> dict:
    ...
    for i, chunk in enumerate(chunks):
        ...
        _memory.add_fact(
            ...
            embed_text=chunk,           # embed full chunk, not just the 120-char descriptor preview
            value={
                "chunk": chunk,
                "chunk_index": i,
...
def search_knowledge(query: str, k: int = 5) -> list[dict]:
    ...
    try:
        # Check if the global memory._index() simulation is active:
        _memory._index()
        ...
        return [
            {
                "status": "error",
                "message": f"Search failed: Vector index database is offline/unavailable ({e})."
            }
        ]
    ...
    return [
        {
            ...
            "source": item.source,
            "chunk_preview": (item.value.get("chunk") or "")[:100000],
