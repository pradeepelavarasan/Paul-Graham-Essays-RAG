...
def _index() -> VectorIndex:
    ...
    """
    # TO SIMULATE GLOBAL INDEX UN-AVAILABILITY: Uncomment the line below:
    # raise RuntimeError("Vector Index Database is offline/unavailable.")
...
def _vector_search(
    ...
    *,
    ...
    try:
        ...
        print(f"[memory.read] Vector search failed/disabled: {e}")
        return []
...
def _persist_item(item: MemoryItem) -> MemoryItem:
    ...
    if item.embedding is not None and item.kind in _EMBEDDABLE_KINDS:
        try:
            ...
            print(f"[memory] indexing failed/disabled ({e!r}); item only written to json")
    return item
...
def add_fact(
    ...
    *,
    ...
    When `embed_text` is provided it is used for the embedding instead of
    `descriptor`, allowing callers (e.g. index_document) to embed the full
    chunk text while keeping the human-readable descriptor short. Without
    this, only the first ~120 chars of each chunk were ever embedded, making
    vector retrieval unreliable for any content past that prefix."""
    text_to_embed = embed_text if embed_text is not None else descriptor
    embedding = _try_embed(text_to_embed, task_type="retrieval_document")
    item = MemoryItem(
        id=new_id("mem"),
        kind="fact",
